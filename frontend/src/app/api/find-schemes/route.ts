...
import { retrieveRelevantSchemes, rankSchemes } from "@/backend/services/SchemeRetrievalService";
...
import { searchCache } from "@/backend/services/SchemeCacheService";
...
import type { RankedScheme } from "@/backend/services/SchemeRetrievalService";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let scoredSchemes: RankedScheme[] = [];
    let fullSchemes: GovernmentScheme[] = [];

    const retrievalResult = await retrieveRelevantSchemes(profile, {
      limit: maxSchemes,
      minScore: minScore,
    });
    ...
    if (fullSchemes.length === 0 && searchQueries.length > 0) {
      ...
      const searchResult = await searchCache(topQuery, {
        limit: maxSchemes,
      });

      if (searchResult.success && searchResult.data.length > 0) {
        fullSchemes = searchResult.data;
        scoredSchemes = rankSchemes(fullSchemes, profile);
      }
    }
    ...
    steps.push({
      ...
      details: `${fullSchemes.length} schemes retrieved, avg relevance: ${retrievalResult.averageScore}%`,
    });
