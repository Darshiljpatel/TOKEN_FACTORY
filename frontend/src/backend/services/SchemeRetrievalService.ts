...
import { discoverSchemes } from "./discoveryService";
...
async function triggerDiscoveryIfNeeded(
  ...
  try {
    ...
    if (!result.success) {
      console.warn("[triggerDiscoveryIfNeeded] Discovery had errors:", result.errors.join("; "));
    }
    // Return only schemes not already retrieved
    return result.schemes.filter(scheme => !existingIds.has(scheme.id));
  } catch (err) {
    console.error("[triggerDiscoveryIfNeeded] Discovery service failed:", err);
    return [];
  }
}
