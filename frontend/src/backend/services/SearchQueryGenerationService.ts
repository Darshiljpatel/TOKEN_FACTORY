...
// Uses Nebius AI to generate optimized web search queries
...
async function callNebiusAPI(profileText: string): Promise<string> {
...
export class SearchQueryGenerationService {
  /**
   * Generates optimized web search queries using Nebius AI.
  ...
  static async generateQueries(profile: UserProfile): Promise<QueryGenResult> {
    try {
      ...
      const rawResponse = await callNebiusAPI(profileText);
