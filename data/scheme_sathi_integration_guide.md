...
graph TD
    ...
    B --> D[Search Supabase Cache SchemeCacheService]
...
The core backend files are integrated within the Next.js application, organized as follows under the `TOKEN_FACTORY/frontend/src` directory:

```bash
frontend/src/
├── app/
│   └── api/                     # Next.js API Routes (e.g., POST /api/retrieve-schemes)
└── backend/                     # Backend Logic Service Layer
    ├── lib/
    │   ├── nebiusClient.ts      # Nebius API connection and prompt logic
    │   ├── supabase.ts          # Supabase Client initializations
    │   └── supabaseClient.ts
    ├── services/
    │   ├── BenefitsAnalysisService.ts
    │   ├── EligibilityCheckerService.ts
    │   ├── ProfileExtractionService.ts
    │   ├── SchemeCacheService.ts      # Cache Reads, Writes, Searching, and Logging
    │   ├── SchemeDiscoveryService.ts
    │   ├── SchemeParser.ts
    │   ├── SchemeRetrievalService.ts  # Core Orchestrator (Cache first -> LLM Discovery fallback)
    │   ├── SearchQueryGenerationService.ts
...
### 1. Multi-Profile Retrieval Test (`frontend/src/backend/tests/testSchemeRetrieval.ts`)
...
### 2. End-to-End Suite (`frontend/src/backend/tests/e2e.integration.ts`)
