...
import { getSupabaseAdmin } from "@/backend/lib/supabaseClient";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await admin
      .from("scheme_cache")
      .upsert(SAMPLE_SCHEMES, { onConflict: "scheme_name" })
      .select();
    ...
    return NextResponse.json({
      success: true,
      message: `Seeded ${data.length} schemes into scheme_cache`,
      schemes: data,
    });
  } catch (err: unknown) {
