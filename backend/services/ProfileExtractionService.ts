...
// Uses Gemini AI to extract structured UserProfile data
...
export class ProfileExtractionService {
   ...
   * Gemini AI, with Zod validation on the output.
   *
  ...
  static async extractWithDefaults(
    userText: string
  ): Promise<{ profile: UserProfile; confidence: number; missingFields: string[] } | ExtractionFailure> {
